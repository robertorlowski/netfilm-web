...
type UninitializedPlayer = Pick<
  Player,
  "videoId" | "destroy" | "addEventListener"
>;
...
export class YouTubePlayer implements AfterViewInit, OnDestroy, OnInit {
  ...
  private _suggestedQuality = new BehaviorSubject<
    YT.SuggestedVideoQuality | undefined
  >(undefined);
  ...
  private _playerVars = new BehaviorSubject<YT.PlayerVars | undefined>(
    undefined
  );

  /** Outputs are direct proxies from the player itself. */
  @Output() ready: Observable<YT.PlayerEvent> = this._getLazyEmitter<
    YT.PlayerEvent
  >("onReady");

  @Output() stateChange: Observable<
    YT.OnStateChangeEvent
  > = this._getLazyEmitter<YT.OnStateChangeEvent>("onStateChange");

  @Output() error: Observable<YT.OnErrorEvent> = this._getLazyEmitter<
    YT.OnErrorEvent
  >("onError");

  @Output() apiChange: Observable<YT.PlayerEvent> = this._getLazyEmitter<
    YT.PlayerEvent
  >("onApiChange");

  @Output() playbackQualityChange: Observable<
    YT.OnPlaybackQualityChangeEvent
  > = this._getLazyEmitter<YT.OnPlaybackQualityChangeEvent>(
    "onPlaybackQualityChange"
  );

  @Output() playbackRateChange: Observable<
    YT.OnPlaybackRateChangeEvent
  > = this._getLazyEmitter<YT.OnPlaybackRateChangeEvent>(
  ...
  ngOnInit() {
    ...
    if (!window.YT) {
      ...
      iframeApiAvailableObs = iframeApiAvailableSubject.pipe(
        take(1),
        startWith(false)
      );
    }
...
function createPlayerObservable(
  ...
  const playerOptions = videoIdObs.pipe(
    ...
    map(([videoId, [width, height, playerVars]]) =>
      videoId ? { videoId, width, height, playerVars } : undefined
    )
  );
...
function skipUntilRememberLatest<T>(
...
function syncPlayerState(
  ...
  [container, videoOptions, ngZone]: [
    HTMLElement,
    YT.PlayerOptions | undefined,
    NgZone
  ]
